...
import numpy as np
def delete_noise_data(FEATURE_DF, col, hanguel_pattern = '[^ㄱ-ㅎ ㅏ-ㅣ 가-힣]'):
    print(f"제거 전: \n{FEATURE_DF[col][:5]}")
    print('-'*70)
    
    # 한글과 공백을 제외하고 모두 제거
    FEATURE_DF[col]= FEATURE_DF[col].str.replace(hanguel_pattern, '', regex = True)
    print(f"hanguel_pattern이외의 문자들 제거 후: \n{FEATURE_DF[col][:5]}")
    print('-' * 70)
